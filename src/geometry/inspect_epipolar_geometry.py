...
def inspect(img1, K1, img2, K2, rel_pose_est, rel_pose_gt):
    ...
    for i in range(len(E)):
        ...
        im1, im2 = drawpointslines((img1[i].cpu().numpy())*255., pts1, (img2[i].cpu().numpy())*255., lines2, colors)

        im1_gt, im2_gt = drawpointslines((img1[i].cpu().numpy())*255., pts1, (img2[i].cpu().numpy())*255., lines2_gt, colors)
        im2_copy = ((img2[i].cpu().numpy())*255.).copy()
        im2_gt_copy =((img2[i].cpu().numpy())*255.).copy()
