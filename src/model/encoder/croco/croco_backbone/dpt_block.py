...
class DPTOutputAdapter(nn.Module):
    ...
    def __init__(self,
                 ...
                 hooks: List[int] = [2, 8, 14, 22], # [0, 6, 9, 12] [2, 5, 8, 11]
                 layer_dims: List[int] = [128, 256, 512, 1024], # [96, 192, 384, 768]
                 feature_dim: int = 256,
                 last_dim: int = 32,
                 use_bn: bool = False,
                 ...
                 **kwargs):
        super().__init__()
        self.num_channels = num_channels # 4 (pts + conf)
        self.stride_level = stride_level # 1
        self.patch_size = pair(patch_size)
        self.main_tasks = main_tasks
        self.hooks = hooks
        self.layer_dims = layer_dims
        self.feature_dim = feature_dim # 256
        self.dim_tokens_enc = dim_tokens_enc * len(self.main_tasks) if dim_tokens_enc is not None else None # None
        self.head_type = head_type # regression
    ...
    def init(self, dim_tokens_enc=768):
        ...
        self.act_4_postprocess = nn.Sequential(
            nn.Conv2d(
                ...
                out_channels=self.layer_dims[3] // 2,
                kernel_size=1, stride=1, padding=0,
            ),
            nn.Conv2d(
                in_channels=self.layer_dims[3] // 2,
                out_channels=self.layer_dims[3] // 2,
    ...
    def forward(self, encoder_tokens: List[torch.Tensor], multi_view_feats: List[torch.Tensor], image_size):
            #input_info: Dict):
