...
class DatasetCambridge(IterableDataset):
    ...
    def make_poses_relative_to_ref(self, ref_extrinsic, extrinsics):
        ref_extr_inv = ref_extrinsic.inverse()
        for i in range(extrinsics.shape[0]):
            extrinsics[i] = torch.bmm(
                ref_extr_inv.unsqueeze(0),
                extrinsics[i].unsqueeze(0),
            ).squeeze(0)
        return extrinsics
